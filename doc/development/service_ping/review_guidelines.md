...
- For a metric's YAML definition:
  ...
  - Check the tiers.
- If a metric was changed or removed: Notify the Customer Success Ops team (`@csops-team`), Analytics Engineers (`@gitlab-data/analytics-engineers`), and Product Analysts (`@gitlab-data/product-analysts`) by `@` mentioning those groups in a comment on the MR.
