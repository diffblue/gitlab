...
| `spam_check_endpoint_enabled`            | boolean          | no                                   | Enables spam checking via external Spamcheck API endpoint. Default is `false`. |
| `spam_check_endpoint_url`                | string           | no                                   | URL of the external Spamcheck service endpoint. This must use a URI scheme of either "grpc" or "tls". Specifying a "tls" scheme will force communication to be encrypted.|
| `spam_check_api_key`                      | string           | no                                   | The API key used by GitLab for accessing the Spamcheck service endpoint. |
