...
class BackfillArchivedFieldInWikis < Elastic::Migration
  ...
  def enqueue_tasks_for_projects(projects_in_progress)
    ...
    projects = Project.id_in(project_ids_to_work_on).inc_routes.select(:id, :namespace_id, :archived)
  ...
  def get_failed_or_completed_projects(projects)
    ...
    projects.each do |item|
      ...
      if task_status['error'].present?
        log_warn("Failed to update wikis",
      ...
      if task_status['completed'].present?
        log("Completed: backfill archived in wikis index",
          project_id: project_id,
          search_task_id: task_id
        )
        failed_or_completed_projects << item
      else
        log("In Progress: backfill archived in wikis index",
          project_id: project_id,
          search_task_id: task_id
        )
      end
  ...
  def update_by_query(project)
    ...
    query[:bool][:filter] = { term: { rid: "wiki_project_#{project.id}" } }
    response = client.update_by_query(
      index: index_name,
      body: { query: query, script: { lang: 'painless', source: "ctx._source.archived = #{project.archived}" } },
      wait_for_completion: false,
      max_docs: batch_size,
      timeout: ELASTIC_TIMEOUT,
      routing: "n_#{project.root_namespace.id}",
      conflicts: 'proceed'
    )
  ...
  def query_missing_field(exclude_project_ids = [])
    { bool: { must_not: [{ exists: { field: 'archived' } }], must: { exists: { field: 'project_id' } } } }.tap do |q|
      unless exclude_project_ids.empty?
        q[:bool][:must_not] << { terms: { rid: exclude_project_ids.map { |id| "wiki_project_#{id}" } } }
      end
    end
  end
