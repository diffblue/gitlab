...
module Geo::ReplicableRegistry
  ...
  BULK_MARK_PENDING_REDIS_KEY = 'geo:latest_id_marked_as_pending'
  ...
  class_methods do
    ...
    def one_batch_to_bulk_mark_pending_relation
      not_pending.ordered.limit(BULK_MARK_PENDING_BATCH_SIZE).where("id > ?", get_bulk_mark_pending_cursor)
    end
    ...
    def bulk_mark_pending_one_batch!
      ...
      last_id = one_batch_to_bulk_mark_pending_relation.maximum(primary_key)

      rows_updated = one_batch_to_bulk_mark_pending_relation
                      .update_all(state: STATE_VALUES[:pending], last_synced_at: nil)
    ...
    def set_bulk_mark_pending_cursor(last_id_updated)
      Gitlab::Redis::SharedState.with do |redis|
        redis.set(BULK_MARK_PENDING_REDIS_KEY, last_id_updated)
      end
    end
    ...
    def get_bulk_mark_pending_cursor
      Gitlab::Redis::SharedState.with do |redis|
        redis.get(BULK_MARK_PENDING_REDIS_KEY).to_i
      end
    end
  end
