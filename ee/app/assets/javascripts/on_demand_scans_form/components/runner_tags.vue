...
export default {
  ...
  data() {
    return {
      text: this.value?.join(', ') || this.$options.i18n.runnerSearchHeader,
      search: '',
      isLoading: false,
      selected: [],
      tags: [],
  ...
  methods: {
    ...
    setSelection(tags) {
      this.selected = tags;
      this.text = this.selected.length
        ? this.selected.join(', ')
        : this.$options.i18n.runnerSearchHeader;
  ...
  <div>
    <gl-sprintf :message="$options.i18n.runnerTagsLabel" class="form-text text-gl-muted gl-mb-5">
      <template #link="{ content }">
        <gl-link
          href="https://docs.gitlab.com/ee/ci/runners/configure_runners.html#use-tags-to-control-which-jobs-a-runner-can-run"
          target="_blank"
          >{{ content }}</gl-link
        >
      </template>
    ...
    <gl-collapsible-listbox
      class="gl-mt-3 gl-display-block"
      :block="true"
