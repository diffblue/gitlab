    ...
    class UserClassProxy < ApplicationClassProxy
      ...
      def elastic_search(query, fuzzy_search_fields: FUZZY_SEARCH_FIELDS, options: {})
        ...
        if options[:count_only]
          filters = fuzzy_query(filters, query, fuzzy_search_fields, options)
          query_hash[:size] = 0
        else
          musts = fuzzy_query(musts, query, fuzzy_search_fields, options)
        end

        filters = ancestry_query(filters, options)
        filters = forbidden_states_filter(filters, options)

        query_hash[:query][:bool][:must] = musts
      ...
      def fuzzy_query(filters, query, fuzzy_search_fields, options)
        return filters unless query

        fuzzy_search_fields -= ['email'] unless admin(options)
        shoulds = []

        fuzzy_search_fields.each do |field|
          shoulds << {
            fuzzy: {
              "#{field}": {
                value: query,
