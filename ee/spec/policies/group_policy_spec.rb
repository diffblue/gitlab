    ...
    context 'when group_saml is available' do
      ...
      context 'with sso enforcement enabled' do
        ...
        context 'when the session has been set globally' do
          ...
          context 'public group and its hierarchy' do
            let_it_be(:public_root_group) do
              create(:group, :public, saml_provider: create(:saml_provider, enabled: true, enforced_sso: true))
            end

            let_it_be(:sub_group_public) { create(:group, :public, parent: public_root_group) }
            let_it_be(:sub_group_private) { create(:group, :private, parent: public_root_group) }
            ...
            before_all do
              public_root_group.add_developer(developer)
              public_root_group.add_owner(owner)
            end

            subject { described_class.new(user, object) }

            context 'for root group' do
              let(:object) { public_root_group }

              context 'for regular group member' do
                let(:user) { developer }

                it 'does not allow to read group' do
                  expect(subject).not_to allow_action(:read_group)
                end
              end

              context 'for a root group owner' do
                let(:user) { owner }

                it 'allows to read group' do
                  expect(subject).to allow_action(:read_group)
                end
              end
              ...
              context 'for a user who is not a group member' do
                let(:user) { non_member }
              ...
              context 'when minimal_access_role feature is available' do
                ...
                context 'for user with minimal access' do
                  let(:user) { create(:user) }

                  before do
                    create(:group_member, :minimal_access, source: public_root_group, user: user)
                  end
            ...
            context 'for a public subgroup' do
              let(:object) { sub_group_public }
            ...
            context 'for a private subgroup' do
              let(:object) { sub_group_private }
