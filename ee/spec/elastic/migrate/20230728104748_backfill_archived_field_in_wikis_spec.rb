...
require File.expand_path('ee/elastic/migrate/20230728104748_backfill_archived_field_in_wikis.rb')

RSpec.describe BackfillArchivedFieldInWikis, :elastic, :sidekiq_inline, feature_category: :global_search do
  let(:version) { 20230728104748 }
  let(:version_mapping_migration) { 20230719211400 }
  let(:helper) { Gitlab::Elastic::Helper.new }
  let(:index_name) { ::Elastic::Latest::WikiConfig.index_name }
  let(:migration) { described_class.new(version) }
  let_it_be(:projects) { create_list(:project, 3, :wiki_repo) }
  ...
  describe '.migrate' do
    ...
    context 'when no data needs to be updated' do
      it 'does not execute update_by_query', :aggregate_failures do
        ...
        projects.each do |p|
          p.wiki.create_page('index_page', 'Bla bla term')
    ...
    context 'when task is in progress' do
      ...
      context 'when the task returns an error' do
        before do
          task_id = migration.migration_state[:projects_in_progress].first[:task_id]
          allow(helper).to receive(:task_status).with(task_id: task_id).and_return({ 'error' =>
  ...
  describe '.completed?' do
    ...
    context 'when wikis are missing archived' do
      before do
        set_elasticsearch_migration_to(version_mapping_migration, including: false)
        projects.each do |p|
          p.wiki.create_page('index_page', 'Bla bla term')
    ...
    context 'when no wikis are missing archived' do
      before do
        set_elasticsearch_migration_to(version_mapping_migration, including: true)
        projects.each do |p|
          p.wiki.create_page('index_page', 'Bla bla term')
  ...
  describe 'integration test' do
    before do
      set_elasticsearch_migration_to(version_mapping_migration, including: false)
      projects.each do |p|
        2.times { |idx| p.wiki.create_page("index_page#{idx}", 'Bla bla term') }
        p.wiki.index_wiki_blobs
      end
      ensure_elasticsearch_index!
    end
    ...
    it 'updates documents in batches', :aggregate_failures do
      ...
      query = { bool: { must: [{ term: { rid: "wiki_project_#{projects.first.id}" } }] } }
      wiki_count = helper.client.count(index: migration.index_name, body: { query: query })['count']
      allow(migration).to receive(:batch_size).and_return((wiki_count / 2.to_f).ceil)
      stub_const("#{described_class}::MAX_PROJECTS_TO_PROCESS", 2)
