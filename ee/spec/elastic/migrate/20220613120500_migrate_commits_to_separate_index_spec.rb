...
RSpec.describe MigrateCommitsToSeparateIndex, feature_category: :source_code_management do
  ...
  describe '.migrate', :elastic_clean, :clean_gitlab_redis_shared_state do
    ...
    context 'batch run' do
      ...
      context 'with commits in elastic', :sidekiq_inline do
        # Create commits on different projects to ensure they are spread across
        # all shards. If they all end up in 1 ES shard then they'll be migrated
        # in a single slice.
        let!(:projects) { create_list(:project, 3, :repository) }

        before do
          projects.each { |project| project.repository.index_commits_and_blobs } # ensure objects are indexed
          ensure_elasticsearch_index!
        end

        it 'migrates all commits' do
          slices = 2
          ...
          expect(es_helper.documents_count(index_name: "#{es_helper.target_name}-commits")).to eq(projects.count * 37)
        end
      end
    end
