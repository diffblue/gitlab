...
RSpec.describe 'Iterations list', :js do
  ...
  context 'as guest' do
    ...
    it 'shows iterations on each tab', :aggregate_failures do
      expect(page).to have_link(current_iteration.period, href: group_iteration_path(project, current_iteration.id))
      expect(page).to have_link(upcoming_iteration.period, href: group_iteration_path(project, upcoming_iteration.id))
      expect(page).not_to have_link(closed_iteration.period)

      click_link('Closed')

      expect(page).to have_link(closed_iteration.period, href: group_iteration_path(project, closed_iteration.id))
