  ...
  describe 'callbacks' do
    ...
    context 'after_save' do
      context 'when plan changes' do
        ...
        let(:namespace_settings) { gitlab_subscription.namespace.namespace_settings }

        before do
          stub_ee_application_setting(should_check_namespace_plan: true)
        end

        context 'with initial free plan' do
          let(:initial_plan) { free_plan }

          it 'is a new free plan' do
            expect(namespace_settings).to be_prevent_sharing_groups_outside_hierarchy
          end

          it 'changes from free plan to paid plan' do
            expect do
              gitlab_subscription.update!(hosted_plan: ultimate_plan)
        ...
        context 'with initial paid plan' do
          let(:initial_plan) { ultimate_plan }

          it 'is a new paid plan' do
            expect(namespace_settings).not_to be_prevent_sharing_groups_outside_hierarchy
          end

          it 'changes from paid plan to free plan' do
            expect do
          ...
          it 'changes from paid plan to paid plan with non-default setting' do
