...
RSpec.describe Analytics::CycleAnalytics::GroupValueStream, type: :model, feature_category: :value_stream_management do
  ...
  describe 'validations' do
    ...
    it 'validates uniqueness of name' do
      group = create(:group)
      create(:cycle_analytics_group_value_stream, name: 'test', namespace: group)

      value_stream = build(:cycle_analytics_group_value_stream, name: 'test', namespace: group)
    ...
    it_behaves_like 'value stream analytics namespace models' do
      let(:factory_name) { :cycle_analytics_group_value_stream }
    end
  end
  ...
  describe 'ordering of stages' do
    ...
    let(:value_stream) do
      create(:cycle_analytics_group_value_stream, namespace: group, stages: [
               create(:cycle_analytics_group_stage, namespace: group, name: "stage 1", relative_position: 5),
               create(:cycle_analytics_group_stage, namespace: group, name: "stage 2", relative_position: nil),
               create(:cycle_analytics_group_stage, namespace: group, name: "stage 3", relative_position: 1)
             ])
    end
