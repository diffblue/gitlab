...
RSpec.describe EE::SecurityOrchestrationHelper, feature_category: :security_policy_management do
  ...
  describe '#scan_result_policy_path' do
    let(:owner) { project.first_owner }
    ...
    context 'when user can modify security policy' do
      before do
        allow(helper).to receive(:can?).with(owner, :modify_security_policy, project) { true }
      end

      it 'returns project security policies path' do
        expect(helper.scan_result_policy_path(project)).to eq(
          "/#{project.full_path}/-/security/policies/new?type=scan_result_policy")
      end
    end

    context 'when user cannot modify security policy' do
      before do
        allow(helper).to receive(:can?).with(owner, :modify_security_policy, project) { false }
      end

      it 'returns new project security policies path' do
        expect(helper.scan_result_policy_path(project)).to eq("/#{project.full_path}/-/security/policies")
      end
    end
  end
  ...
  describe '#show_license_check_settings_alert?' do
    subject { helper.show_license_check_settings_alert?(project) }
    ...
    context 'when approval_rules exist' do
      before do
        create(:approval_project_rule, :license_scanning, project: project)
      end

      context 'when user is on selected page' do
        using RSpec::Parameterized::TableSyntax

        where(:current_path, :show_alert) do
          'projects/settings/merge_requests#show'   | true
          'projects/settings/merge_requests#update' | true
          'projects#show'                           | true
          'projects#edit'                           | true
          'projects#activity'                       | true
        end

        with_them do
          before do
            allow(helper).to receive(:current_path?).and_return(false)
            allow(helper).to receive(:current_path?).with(current_path) { true }
          end

          it 'ignores current path' do
            is_expected.to eq(show_alert)
          end
        end
      end
    ...
    context 'when approval_rules does not exist' do
      it { is_expected.to be_falsey }
    end
  end

  describe '#show_license_check_alert?' do
    subject { helper.show_license_check_alert?(project) }
    ...
    context 'when approval_rules exist' do
      ...
      context 'when user is on selected page' do
        ...
        where(:current_path, :show_alert) do
          'projects/settings/merge_requests#show'   | false
          'projects/settings/merge_requests#update' | false
          'projects#show'                           | true
          'projects#edit'                           | true
          'projects#activity'                       | true
        end
        ...
        with_them do
          ...
          it { is_expected.to eq(show_alert) }
        end
      end
