...
RSpec.describe API::Members, feature_category: :subgroups do
  ...
  context 'group members endpoints for group with minimal access feature' do
    ...
    describe 'PATCH /groups/:id/members/:user_id/member_role', feature_category: :authentication_and_authorization do
      ...
      let_it_be(:member) { create(:group_member, :guest, source: group_with_member_roles) }
      ...
      let(:params) { { member_role_id: member_role.id } }
      let(:user_id) { member.user_id }

      subject(:patch_member_role) do
        patch api("/groups/#{group_id}/members/#{user_id}/member_role", current_user),
          params: params
      end
      ...
      context "when customizable_roles feature flag is enabled" do
        ...
        context "when an invalid group_id is passed" do
        ...
        context "when current_user is an owner of the group" do
          context "when member's group access level matches the access level on the member role" do
            let(:params) { { member_role_id: member_role.id } }

            it "updates the member to associate it with the passed member_role" do
              patch_member_role
          ...
          context "when member is a non-guest member of the group" do
            let_it_be(:member) { create(:group_member, :developer, source: group_with_member_roles) }
            ...
            it "raises an error" do
              ...
              expect(json_response['message']).to eq(
                "Member role role's base access level does not match the access level of the membership"
              )
            end
          end
          ...
          context "when invalid user id is passed" do
            ...
            it "returns 404" do
              ...
              expect(response).to have_gitlab_http_status(:not_found)
              expect(json_response['message']).to eq('404 Member Not Found')
            end
          end

          context "when invalid member_role id is passed" do
            let(:params) { { member_role_id: non_existing_record_id } }
