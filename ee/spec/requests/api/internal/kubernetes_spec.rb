...
RSpec.describe API::Internal::Kubernetes do
  ...
  describe 'GET /internal/kubernetes/modules/starboard_vulnerability/scan_execution_policies' do
    def send_request(headers: {})
      get api('/internal/kubernetes/modules/starboard_vulnerability/scan_execution_policies'), headers: headers.reverse_merge(jwt_auth_headers)
    end
    ...
    context 'when security_orchestration_policies is licensed' do
      ...
      context 'when policies are present' do
        ...
        it 'returns expected data', :aggregate_failures do
          ...
          expect(response).to have_gitlab_http_status(:success)
          expect(json_response['policies']).to match_array([
                                                 {
      ...
      context 'when policies are empty' do
        it 'returns empty array', :aggregate_failures do
          ...
          expect(response).to have_gitlab_http_status(:success)
          expect(json_response['policies']).to be_empty
        end
      end
    end
  end
