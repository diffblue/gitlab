...
RSpec.describe Emails::ComplianceViolations, feature_category: :compliance_management do
  include EmailSpec::Matchers

  include_context 'gitlab email notification'

  describe '#compliance_violations_csv_email', travel_to: '2022-02-24' do
    ...
    let(:filename) { "2022-02-24-33-violations.csv" }
    let(:content_type) { 'text/csv' }
    let(:csv_data) { 'csv,separated,things' }

    let(:expected_text) do
      'Your Compliance Violations CSV export for the group "%{group_name}" has been attached to this email.'
    end

    let(:expected_html) do
      'Your Compliance Violations CSV export for the group %{group_link} has been attached to this email.'
    end
