...
RSpec.describe Notify, feature_category: :not_owned do # rubocop:disable RSpec/InvalidFeatureCategory
  ...
  context 'for a project' do
    ...
    context 'for merge request with reviewer' do
      describe 'merge request diff summary' do
        let_it_be(:merge_request_diff) { create(:merge_request_diff, merge_request: merge_request) }
        let_it_be(:recipient) { create(:user) }

        subject { described_class.request_review_merge_request_email(recipient.id, merge_request.id, merge_request.author_id) }
        ...
        context 'when merge_request_diff_llm_summary exists' do
          let_it_be(:merge_request_diff_llm_summary) { create(:merge_request_diff_llm_summary, merge_request_diff: merge_request_diff) }

          context 'when SummarizeDiffService is disabled' do
            let(:summarize_diff_service_enabled) { false }

            it 'does not include diff summary' do
              expect(subject.text_part.body.raw_source).not_to include(merge_request_diff_llm_summary.content)
            end
          end
          ...
          context 'when SummarizeDiffService is enabled' do
            ...
            it 'includes diff summary' do
              expect(subject.text_part.body.raw_source).to include(merge_request_diff_llm_summary.content)
              expect(subject.text_part.body.raw_source).to include('Summary generated by AI (Experiment)')
            end
          end
        end

        context 'when merge_request_diff_llm_summary is empty' do
          context 'when SummarizeDiffService is enabled' do
            let(:summarize_diff_service_enabled) { true }

            it 'does not include diff summary' do
              expect(subject.text_part.body.raw_source).not_to include('Summary generated by AI (Experiment)')
            end
          end
        end
      end
