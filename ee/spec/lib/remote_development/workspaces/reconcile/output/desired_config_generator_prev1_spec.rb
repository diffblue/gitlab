...
RSpec.describe RemoteDevelopment::Workspaces::Reconcile::Output::DesiredConfigGeneratorPrev1, :freeze_time, feature_category: :remote_development do
  ...
  describe '#generate_desired_config' do
    ...
    let(:workspace) do
      instance_double(
        ...
        processed_devfile: read_devfile('example.processed-devfile-prev1.yaml'),
        user: user,
        agent: agent,
        config_version: 1
      )
    ...
    context 'when DevfileParser returns empty array' do
      before do
        allow(RemoteDevelopment::Workspaces::Reconcile::Output::DevfileParserPrev1).to receive(:get_all).and_return([])
      end
