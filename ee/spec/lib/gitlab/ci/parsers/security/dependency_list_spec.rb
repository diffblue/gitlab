...
RSpec.describe Gitlab::Ci::Parsers::Security::DependencyList, feature_category: :dependency_management do
  ...
  describe '#apply_licenses!' do
    context 'without indirect dependencies' do
      ...
      context 'with existing license' do
        ...
        it 'applies license to dependency' do
    ...
    context 'for multi license report with indirect dependency whose ancestor is invalid' do
      let(:indirect_dependency) { build :dependency, :indirect_with_invalid_ancestor, :with_vulnerabilities, name: 'Library1' }
      let(:multi_license_report) { build(:ci_reports_license_scanning_report, :report_1) }
      ...
      it 'applies license' do
        parser.apply_licenses!(multi_license_report, report)

        expect { dependencies }.not_to raise_error
        expect(dependencies.count).to eq(1)
        expect(dependencies.first[:licenses][0][:name]).to eq('MIT')
      end

      context 'and also includes a direct dependency' do
        let(:direct_dependency) { build :dependency, :direct, name: 'Library2' }

        before do
          report.add_dependency(direct_dependency)
        end

        it 'applies licenses' do
          parser.apply_licenses!(multi_license_report, report)

          expect { dependencies }.not_to raise_error
          expect(dependencies.count).to eq(2)
          expect(dependencies.first[:licenses][0][:name]).to eq('MIT')
          expect(dependencies.last[:licenses][0][:name]).to eq('WTFPL')
        end
      end
    end
  end
