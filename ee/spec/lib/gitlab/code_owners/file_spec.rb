...
RSpec.describe Gitlab::CodeOwners::File, feature_category: :source_code_management do
  ...
  # TODO: remove these shared examples with codeowners_default_owners FF
  shared_examples 'CodeOwners::File' do
        ...
        shared_examples_for "creates expected parsed sectional results" do
          ...
          with_them do
            ...
            it "assigns the correct owners for each entry" do
              ...
              if Feature.enabled?(:codeowners_default_owners)
                expect(extracted_owners).to contain_exactly(*owners)
              else
                expect(extracted_owners).to contain_exactly(*owners_ff_disabled)
              end
            end
          end
        end
  ...
  it_behaves_like 'CodeOwners::File'

  context 'when codeowners_default_owners FF is disabled' do
    before do
      stub_feature_flags(codeowners_default_owners: false)
    end
