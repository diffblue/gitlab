...
RSpec.describe Gitlab::Usage::Metrics::Instrumentations::InProductMarketingEmailMetric do
  using RSpec::Parameterized::TableSyntax

  let(:email_attributes) { { cta_clicked_at: Date.yesterday, track: 'verify', series: 0 } }
  let(:options) { { only_clicked: true, track: 'verify', series: 0 } }
  ...
  it_behaves_like 'a correct instrumented metric value and query', {
    options: { only_clicked: true, track: 'verify', series: 0 },
    time_frame: 'all'
  }

  where(:options_key, :valid_value, :invalid_value) do
    :only_clicked | true           | 'yes'
