...
RSpec.describe Ci::Pipeline, :mailer, factory_default: :keep do
  ...
  describe '.with_reports' do
    context 'when pipeline has a test report' do
      subject { described_class.with_reports(Ci::JobArtifact.for_report(:test)) }
    ...
    context 'when pipeline has a coverage report' do
      subject { described_class.with_reports(Ci::JobArtifact.for_report(:coverage)) }
    ...
    context 'when pipeline has an accessibility report' do
      subject { described_class.with_reports(Ci::JobArtifact.for_report(:accessibility)) }
    ...
    context 'when pipeline has a codequality report' do
      subject { described_class.with_reports(Ci::JobArtifact.for_report(:codequality)) }
    ...
    context 'when pipeline has a terraform report' do
      it 'selects the pipeline' do
        ...
        expect(described_class.with_reports(Ci::JobArtifact.for_report(:terraform))).to eq(
  ...
  describe '#latest_report_builds' do
    ...
    it 'filters builds by scope' do
      ...
      expect(pipeline.latest_report_builds(Ci::JobArtifact.for_report(:test))).to contain_exactly(test_build)
    end
  ...
  describe '#latest_report_builds_in_self_and_descendants' do
    ...
    it 'filters builds by scope' do
      ...
      expect(pipeline.latest_report_builds_in_self_and_descendants(Ci::JobArtifact.for_report(:codequality))).to contain_exactly(grandchild_build)
    end
  ...
  describe '#has_reports?' do
    subject { pipeline.has_reports?(Ci::JobArtifact.for_report(:test)) }
