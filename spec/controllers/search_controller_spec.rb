    ...
    describe '#append_info_to_payload' do
      it 'appends search metadata for logging' do
        expect(controller).to receive(:append_info_to_payload).and_wrap_original do |method, payload|
          ...
          expect(payload[:metadata]['meta.search.project_ids']).to eq('true')
        end

        get :show, params: { scope: 'issues', search: 'hello world', group_id: '123', project_id: '456',project_ids:['456','789'], confidential: true, state: true, force_search_results: true }
      end
