...
RSpec.shared_examples_for 'a panel without placeholders in EE' do
  # EE sidebars are a superset of CE sidebars.
  # There might be placeholders in CE, which would only be filled in EE
  # By checking that EE has no placeholders, we implicitly checked that everything
  # is alright in CE
  if Gitlab.ee?
    let(:menu_items) do
    ...
    it 'has no Sidebars::NilMenuItem in EE' do
